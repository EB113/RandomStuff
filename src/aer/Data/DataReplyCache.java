...
public class DataReplyCache {
    ...
    HashMap <ByteArray, HashMap<ByteArray, ByteArray>> gemSearch;
    Config config;

    DataReplyCache(Config config) {
        this.config     = config;
        this.gem        = new HashMap<ByteArray, HashMap<ByteArray, Info>>();
        this.gemSearch  = new HashMap<ByteArray, HashMap<ByteArray, ByteArray>>();
    }
    

    boolean addRep(byte[] src_old, byte[] dst_old, byte[] req_num_old, long ttl, LinkedList<InetAddress> usedPeers, byte[] raw) {
        
        ByteArray src       = new ByteArray(src_old);
        ByteArray dst       = new ByteArray(dst_old);
        ...
        if(this.gem.containsKey(src)) {//SE JA TEM
            ...
            if(tmpMap.containsKey(req_num)){
                ...
                Info tmp = new Info(usedPeers, dst_old, ttl, raw);
                tmpMap.put(req_num, tmp);
    ...
    ArrayList<byte[]> getRep(byte[] nodeID, boolean mode) {
        Info info               = null;
